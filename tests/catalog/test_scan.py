...
"""
            ...
            with patch("ast.parse") as mock_parse:
                ...
                with patch("catalog.scan.EnvVarVisitor") as mock_visitor_class:
                    ...
                    catalog, total = scan_codebase(
                        base_dir=".", exclude_dirs=["venv", "tests"], exclude_patterns=["test_"], no_auto_tag=False
                    )
                    ...
                    api_url_key = "API_URL_https://api.example.com"
                    catalog_idx = -1
                    ...
                    assert catalog[catalog_idx]["has_default"]
                    ...
                    assert not catalog[catalog_idx]["has_default"]
    ...
    def test_scan_codebase_with_auto_tag_disabled(self, mock_file_system, mock_env_var_content):
        with patch("builtins.open", side_effect=mock_env_var_content):
            with patch("ast.parse"), patch("catalog.scan.EnvVarVisitor") as mock_visitor_class:
                ...
                catalog, total = scan_codebase(
                    base_dir=".", exclude_dirs=["venv"], exclude_patterns=[], no_auto_tag=True
                )
    ...
    def test_scan_codebase_error_handling(self, mock_file_system):
        with patch("builtins.open", side_effect=IOError("File read error")), patch(
            "builtins.print"
        ) as mock_print, pytest.raises(IOError):
            scan_codebase(base_dir=".", exclude_dirs=[], exclude_patterns=[], no_auto_tag=False)
    ...
    def test_scan_codebase_exclude_patterns(self, mock_file_system, mock_env_var_content):
        with patch("builtins.open", side_effect=mock_env_var_content):
            with patch("ast.parse"), patch("catalog.scan.EnvVarVisitor") as mock_visitor_class:
                ...
                catalog, total = scan_codebase(
                    base_dir=".", exclude_dirs=["venv"], exclude_patterns=["utils.py"], no_auto_tag=False
                )
