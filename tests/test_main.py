...
class TestCreateEnvVarCatalogue:
    ...
    def test_create_env_var_catalogue(self, mock_json_dump, mock_file, mock_scan_codebase):
        ...
        mock_scan_codebase.assert_called_once_with(
            ".",
            ["venv", "__pycache__"],
            ["*.pyc"],
            False
...
class TestUpdateEnvVarCatalogue:
    ...
    def test_update_env_var_catalogue(self, mock_exists, mock_json_dump, mock_file, mock_load_catalog, mock_scan_codebase):
        ...
        # Call the function
        update_env_var_catalogue(
            output_file="updated_catalog.json",
            exclude_dirs=["venv"],
            exclude_patterns=[],
            no_auto_tag=True
        )
...
class TestCheckEnvVars:
    ...
    def test_check_env_vars_missing_vars(self, mock_print_structured, mock_print,
                                        mock_json_load, mock_open, mock_scan_codebase):
        # Mock catalog loading
        mock_json_load.return_value = [
            {"name": "KNOWN_VAR", "has_default": True, "default_value": "default"}
        ]
...
class TestMain:
    ...
    def test_main_tags_from_env(self, mock_save, mock_add_tags, mock_load):
        catalog_data = [
            {"name": "TEST_VAR1", "tags": []},
            {"name": "TEST_VAR2", "tags": ["existing"]}
        ]
        mock_load.return_value = catalog_data

        updated_catalog = [
            {"name": "TEST_VAR1", "tags": ["prod"]},
            {"name": "TEST_VAR2", "tags": ["existing", "prod"]}
        ]
